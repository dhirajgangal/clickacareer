<script src="<?php echo base_url();?>assets/ckeditor/ckeditor.js"></script>
<script src="<?php echo base_url();?>assets/ckeditor/samples/js/sample.js"></script>
<link rel="stylesheet" href="<?php echo base_url();?>assets/ckeditor/samples/css/samples.css">
<link rel="stylesheet" href="<?php echo base_url();?>assets/ckeditor/samples/toolbarconfigurator/lib/codemirror/neo.css">
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
<script>
    $(function() {
  $('input[name=company_name]').on('input',function() {
    var selectedOption = $('option[value="'+$(this).val()+'"]');
    console.log(selectedOption.length ? selectedOption.attr('id') : 'This opiton is not in the list!');
    document.getElementById('companyid').value=selectedOption.attr('id');
   
//    alert(selectedOption.attr('id'));
  });
});
</script>
<section id="content">
          <section class="vbox">          
            <section class="scrollable padder">
              <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                <li><a href="index.html"><i class="fa fa-pencil"></i> Edit Job</a></li>
<!--                <li class="active">Workset</li>-->
              </ul>
      <section class="panel panel-default m-t-lg bg-white">
        <header class="panel-heading text-center">
          <strong>Edit Job Form</strong>
        </header>
          <form action="<?php echo BASE_URL; ?>adminclickacareer/submit_edit_job" method="POST" enctype="multipart/form-data" class="panel-body wrapper-lg">
          <?php foreach ($objcetgetgeteditjob as $value) { ?>
     
              
           <div class="form-group  col-md-4">
            <label class="control-label">Title</label> <span id="alreadyexists" style="color: red" ></span>
            <input type="hidden" name="hdn_job_id" id="hdn_job_id" value="<?php echo $value->id;?>" />
            <input type="text" name="title" id="" value="<?php echo $value->title;?>" placeholder="Title" class="form-control input-lg" required="">
          </div>
          <div class="form-group  col-md-4">
            <label class="control-label">Company Name</label>
            <input type="hidden" id="companyid" name="company_details" value="<?php echo $value->company_details;?>" placeholder=" Company details " class="form-control input-lg" required="">
          <input list="browsers" id="cname" name="company_name" value="<?php echo $value->name;?>" placeholder=" Select Company Name" class="form-control input-lg" required="">
          <datalist id="browsers">
              <?php 
              foreach ($objcetgetautocompany as $values) {?>
              <option id="<?php echo $values->id;?>" value="<?php echo $values->name;?>">
           <?php  } ?>
          </datalist>
          </div>
<!--          <div class="form-group  col-md-4">
            <label class="control-label">Company details</label>
            <input type="text" name="company_details" value="<?php // echo $value->company_details;?>" placeholder=" Company details " class="form-control input-lg" required="">
          </div>-->
            <div class="form-group  col-md-4">
            <label class="control-label">Experience range</label>
            <input type="text" name="experience_range" value="<?php echo $value->experience_range;?>" placeholder=" Experience range" class="form-control input-lg"  required="">
          </div>
            <div class="form-group  col-md-4">
            <label class="control-label">Salary range</label>
            <input type="text" name="salary_range" value="<?php echo $value->salary_range;?>" placeholder=" Salary range" class="form-control input-lg"  required="">
          </div>
               <div class="form-group  col-md-4">
            <label class="control-label">Location</label>
            <input type="text" name="location" value="<?php echo $value->location;?>" placeholder=" Location" class="form-control input-lg" required="">
          </div>
               <div class="form-group  col-md-4">
            <label class="control-label">Skills</label>
            <input type="text" name="skills" value="<?php echo $value->skills;?>" placeholder=" Skills" class="form-control input-lg"  required="">
          </div>
              <div class="form-group  col-md-12">
            <label class="control-label">Jobs Short description</label>
            <input type="text" name="jobs_short_description" placeholder=" Jobs Short description" value="<?php echo $value->jobs_short_description;?>" required="" class="form-control input-lg">
          </div>
            <div class="form-group  col-md-12">
            <label class="control-label">Long Description</label>
            <textarea id="editor" name="long_description" placeholder=" Long Description" class="form-control input-lg" required=""><?php echo $value->long_description;?></textarea>
          </div>
              <div class="form-group  col-md-6">
            <label class="control-label">Job Location</label>
            <input type="text" name="job_location" value="<?php echo $value->job_location;?>" placeholder=" Job Location" class="form-control input-lg"  required="">
          </div>
              <div class="form-group  col-md-6">
            <label class="control-label">Industry</label>
            <input type="text" name="industry" value="<?php echo $value->industry;?>" placeholder=" Industry" class="form-control input-lg" required="">
          </div>
            <?php }?>
<!--          <div class="checkbox form-group col-md-12">
            <label>
              <input type="checkbox"> Agree the <a href="#">terms and policy</a>
            </label>
          </div>-->
          <button type="submit" class="btn btn-primary">Edit Job</button>
          <div class="line line-dashed"></div>
          <!--<p class="text-muted text-center"><small>Already have an account?</small></p>-->
          <!--<a href="<?php // echo site_url();?>/medicodose/dashboard" class="btn btn-default btn-block">Back</a>-->
        </form>
      </section>
    </div>
    <script>
	initSample();
</script>
    <script>
    function get_user_id(searchKey) {
        //alert(searchKey);
//        var str = searchKey;
        $.ajax({
            url: "<?php echo BASE_URL; ?>medicodose/get_user_id_exists",
            type: 'POST',
            dataType: 'json',
            data: {value: searchKey},
            success: function (data) {
//                alert(data.status);
                if (data.status) {
                    $("#hdn_user_id").val(data.userData.user_id);
                    if(data.userData.user_id!=''){
                      $('#alreadyexists').html('* User id already exists');  
                    }else{
                        $('#alreadyexists').html(); 
                    }
                } else {
//                    alert('1');
                      document.getElementById("hdn_user_id").value = '';
                      document.getElementById("alreadyexists").innerHTML = '';
                    // Some code to run when nothing is found
                }
            }
        });
      }
      </script>
  <!--</section>-->
